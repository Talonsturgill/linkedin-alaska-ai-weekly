...
- **Image.** The `alaska-ai-artwork` skill at
  ...
  template is the emergency fallback only.
...
- **Gmail draft helper.** `scripts/gmail_draft.py` builds the HTML body.
  Pass `--label "The Stack"` and `--footer-label "The Stack"` so the email
  is branded for this column. It prints a JSON payload (subject, to,
  html_body) you pass to the Gmail MCP `create_draft` tool. **HARD RULE:
  the email always carries the post image via a PERMANENT hosted URL** —
  the commit-SHA-pinned `raw.githubusercontent.com` link, verified HTTP
  200 BEFORE the draft is created, rendered inline AND printed as a
  visible clickable link line under the image (`--image-url` does both).
  Base64 (`--image`) is a fallback ONLY when the artifact push itself
  failed after retries, flagged in the Editor's note. Never create a
  draft whose image the human cannot open directly from the email, and
  never write notes implying the image needs a merge to become visible.
...
- The `.claude/settings.json` SessionStart hook runs `pip install -q -r
  ...
  PyYAML, python-dateutil, opensimplex, shapely, coloraide. If the hook fails, the session fails to start,
...
and the "mechanisms already anatomized" reminder. Scouts run on Opus
...
>= 8.5) → technical QA. The Stack loves the diagram-as-art register
(exploded isometrics, cutaways, schematic beauty) but the dedup
cooldowns rule — never repeat a recent style family.

Column parameters:
- Kicker mark on the art: `stack_kicker` from `state.yaml`
  (`"THE STACK"`). Middle slot: the `CATEGORY` string from Phase 4
  (`FACILITIES`, `VEHICLES`, `SOVEREIGNTY`, or `REGULATORY`). Date:
  today in `D MMM YYYY` all caps (America/Anchorage so UTC doesn't
  slip the date).
...
concept the piece on the column's mission itself — the quiet week, the
watch continuing, the machinery at rest — with the same quality bar
and dedup rules, headline something honest like "NO TARGET THIS
CYCLE".
...
python .claude/skills/alaska-ai-artwork/qa_check.py \
  --image out/post_image.png --date "D MMM YYYY" --column "THE STACK"
```
...
4. **Build the payload with the verified URL.** Pass
   `--label "The Stack"` AND `--footer-label "The Stack"`:
...
the image with its visible permanent URL line, Sources (the mechanism's
news trigger and every layer's primary source URL as a bulleted
clickable list), the scorer's report card as a small table, an Editor's
note (anything the editor or scorer flagged, plus any subagent stall,
artwork eval-bar miss or fallback, window broadening this run), and a
footer with run timestamp and branch name.
...
--image-url "$IMG_URL"`), the mapper's `_validation_note`, and the
...
`the stack — {YYYY-MM-DD} — draft id` and push with the same backoff
loop. The artifacts themselves already landed in Phase 9 step 1.
...
- **No-target honesty.** If the mapper clears no mechanism through the
  ...
  skip the post (the artwork still renders per Phase 8, concepted on
  the column's mission), and ship the honest no-target email so the
...
3. `out/post_image.png` exists EVERY cycle (no-target included), is a
   ...
   `THE STACK`, middle slot `<CATEGORY>`). `out/art_plan.md` and
...
- Subagent model assignment lives in each agent's `.md` frontmatter,
  not here. ALL subagents (scouts, mapper, writer, editor, scorer) run
  on Opus — do not downgrade any of them to a smaller model. Writer and
  editor also on
